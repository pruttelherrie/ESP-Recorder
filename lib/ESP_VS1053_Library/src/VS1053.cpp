...
uint16_t VS1053::recordedWordsWaiting(void) {
  return read_register(VS1053_REG_HDAT1);
}

uint16_t VS1053::recordedReadWord(void) {
  return read_register(VS1053_REG_HDAT0);
}

void VS1053::stopRecordOgg(void) {
  write_register(SCI_AICTRL3, 1);
}
...
void VS1053::startRecordOgg(boolean mic) {
  ...
  if (mic) {
    write_register(SCI_MODE,                        VS1053_MODE_SM_ADPCM     | SM_SDINEW);
  } else {
    write_register(SCI_MODE, VS1053_MODE_SM_LINE1 | VS1053_MODE_SM_ADPCM | 2 | _BV(SM_SDINEW) ); // 2 = VU stereo mode
  }

  //if(mic) 
  write_register(SCI_AICTRL0, 1024 );
  if(!mic)   write_register(SCI_AICTRL0, 0x8080 );
...
uint16_t VS1053::loadPlugin(char *plugname) {
 ...
 // Serial.print("Patch size: "); Serial.println(patchsize);
  while ((type = plugin.read()) >= 0) {
    ...
    // set address
    write_register(SCI_WRAMADDR, addr + offsets[type]);
    // write data
    do {
      uint16_t data;
...
uint16_t VS1053::loadSpiffsPlugin(char *plugname) {

  File plugin = SPIFFS.open(plugname);
  if (!plugin) {
    Serial.println("Couldn't open the plugin file");
    Serial.println(plugin);
    return 0xFFFF;
...
boolean VS1053::prepareRecordOgg(char *plugname) {
  write_register(SCI_CLOCKF, 0xC000);  // set max clock
  delay(1); 
  await_data_request();

  write_register(SCI_BASS, 0);  // clear Bass
  ...
  write_register(SCI_WRAMADDR, VS1053_INT_ENABLE);
  write_register(SCI_WRAM, 0x02);
...
boolean VS1053::prepareRecordSpiffs(char *plugname) {
  write_register(SCI_CLOCKF, 0xC000);  // set max clock
  delay(1); 
  await_data_request();
  ...
  int pluginStartAddr = loadSpiffsPlugin(plugname);
  if (pluginStartAddr == 0xFFFF) return false;
  Serial.print("Plugin at $"); Serial.println(pluginStartAddr, HEX);
  if (pluginStartAddr != 0x34) return false;
...
uint32_t VS1053::recordingTime() {

    write_register(SCI_WRAMADDR, 0x8); // Start reading from WRAM
    uint16_t lower = read_register(SCI_WRAM);        // Read back result
    uint16_t higher = read_register(SCI_WRAM);        // Read back result
    
    return higher<<16 | lower;
}

void VS1053::startLevel() {
    write_register(SCI_AICTRL0,0x8080);
}
