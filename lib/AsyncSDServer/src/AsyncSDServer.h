...
class AsyncSDIndexResponse: public AsyncAbstractResponse {
  private:
    File _content;
    String _path;
    void _setContentType(const String& path);
    bool _sourceIsValid;
  public:
    AsyncSDIndexResponse(fs::FS &fs, const String& path, const String& contentType=String(), bool download=false);
    AsyncSDIndexResponse(File content, const String& path, const String& contentType=String(), bool download=false);
    ~AsyncSDIndexResponse();
    bool _sourceValid() const { return _sourceIsValid; } 
    virtual size_t _fillBuffer(uint8_t *buf, size_t maxLen) override;
};
